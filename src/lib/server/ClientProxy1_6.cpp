...
{
  LOG_DEBUG("[CLIP-WS-CP-001] ClientProxy1_6.cpp:29 setClipboard() -- server pushing data to client, id=%d, dirty=%d", id, m_clipboard[id].m_dirty);
  ...
  if (m_clipboard[id].m_dirty) {
    ...
    LOG_DEBUG("[CLIP-WS-CP-002] ClientProxy1_6.cpp:setClipboard() -- sending clipboard data, id=%d target=%s marshalled_size=%zu", id, getName().c_str(), data.size());
...
bool ClientProxy1_6::recvClipboard()
{
  ...
  LOG_DEBUG("[CLIP-WS-CP-005] ClientProxy1_6.cpp:46 recvClipboard() -- receiving clipboard from client");
  ...
  if (auto r = ClipboardChunk::assemble(getStream(), dataCached, id, seq); r == TransferState::Started) {
    ...
    LOG_DEBUG("[CLIP-WS-CP-008] ClientProxy1_6.cpp:69 recvClipboard() -- posting ClipboardChanged event to Server, id=%d seq=%u", id, seq);
