...
void Server::switchScreen(BaseClientProxy *dst, int32_t x, int32_t y, bool forScreensaver)
{
  ...
  if (dst != m_active && m_switchDebounceTimer.getTime() < 0.5) {
    LOG_DEBUG("[CLIP-CU-S-DEBOUNCE] ignoring rapid switch (%.0fms) from %s to %s",
  ...
  if (m_active != dst) {
    ...
    if (m_active == m_primaryClient && m_enableClipboard) {
      LOG_DEBUG("[CLIP-CU-S-016] Server.cpp:switchScreen() -- leaving primary, push clipboard, active=%s", m_active ? m_active->getName().c_str() : "null");
    ...
    LOG_DEBUG("[CLIP-CU-S-017] Server.cpp:switchScreen() -- entering new screen, sending clipboard, active=%s", m_active ? m_active->getName().c_str() : "null");
...
{
  LOG_DEBUG("[CLIP-CU-S-001] Server.cpp:1154 handleClipboardGrabbed() -- event received, grabber=%s", getName(grabber).c_str());
  ...
  LOG_DEBUG("[CLIP-CU-S-004] handleClipboardGrabbed() -- setting new owner, grabber=%s id=%d", getName(grabber).c_str(), info->m_id);
  ...
  LOG_DEBUG("[CLIP-CU-S-005] handleClipboardGrabbed() -- clearing clipboard cache, id=%d", info->m_id);
  ...
  LOG_DEBUG("[CLIP-CU-S-006] handleClipboardGrabbed() -- notifying all clients of grab, id=%d", info->m_id);
  ...
  if (grabber == m_primaryClient) {
    LOG_DEBUG("[CLIP-CU-S-007] handleClipboardGrabbed() -- primary grabbed, re-reading clipboard data");
    for (ClipboardID id = 0; id < kClipboardEnd; ++id) {
...
{
  LOG_DEBUG("[CLIP-CU-S-008] Server.cpp:1208 handleClipboardChanged() -- event received, client=%s", getName(client).c_str());
  // ignore events from unknown clients
  if (!m_clientSet.contains(client)) {
    return;
  }
...
{
  LOG_DEBUG("[CLIP-CU-S-009] Server.cpp:1427 onClipboardChanged() -- sender=%s id=%d seq=%u", getName(sender).c_str(), id, seqNum);
  ...
  LOG_DEBUG("[CLIP-CU-S-011] Server.cpp:onClipboardChanged() -- reading clipboard data from sender, id=%d", id);
  // get data
  ...
  if (clipboard.m_clipboard.has(IClipboard::Format::Text)) {
    ...
    LOG_DEBUG("[CLIP-CU-S-012] Server.cpp:onClipboardChanged() -- text content='%s' (len=%zu)", text.c_str(), text.size());
  }
  ...
  LOG_DEBUG("[CLIP-CU-S-014] Server.cpp:onClipboardChanged() -- new data, updating cache, size=%zu", data.size());
  // got new data
  ...
  LOG_DEBUG("[CLIP-CU-S-015] Server.cpp:onClipboardChanged() -- pushing to active screen, id=%d, active=%s", id, m_active ? m_active->getName().c_str() : "null");
