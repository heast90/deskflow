...
{
  LOG_DEBUG("[CLIP-CU-PC-001] PrimaryClient.cpp:77 getClipboard() -- reading from primary screen, id=%d", id);
  bool result = m_screen->getClipboard(id, clipboard);
  LOG_DEBUG("[CLIP-CU-PC-003a] getClipboard() -- result=%d, clipboard=%d", result, !!clipboard);
  if (result && clipboard) {
    for (int f = 0; f < (int)IClipboard::Format::TotalFormats; f++) {
      ...
      if (has) {
        ...
        LOG_DEBUG("[CLIP-CU-PC-003b] getClipboard() -- format=%d hasData size=%zu text='%s'", f, txt.size(), txt.c_str());
      }
    }
  }
  return result;
...
{
  LOG_DEBUG("[CLIP-CU-PC-002] PrimaryClient.cpp:116 setClipboard() -- writing to primary screen, id=%d, dirty[0]=%d dirty[1]=%d", id, m_clipboardDirty[0], m_clipboardDirty[1]);
  // ignore if this clipboard is already clean
  if (m_clipboardDirty[id]) {
    // this clipboard is now clean
    m_clipboardDirty[id] = false;

    LOG_DEBUG("[CLIP-CU-PC-004] PrimaryClient.cpp:124 setClipboard() -- calling m_screen->setClipboard(), clipboard=%p", (void*)clipboard);
    // set clipboard
...
{
  LOG_DEBUG("[CLIP-CU-PC-005] PrimaryClient.cpp:128 grabClipboard() -- primary clipboard invalidated, id=%d", id);
  // grab clipboard
