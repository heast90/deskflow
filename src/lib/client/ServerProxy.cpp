...
ServerProxy::ConnectionResult ServerProxy::parseMessage(const uint8_t *code)
{
  ...
  if (memcmp(code, kMsgDMouseMove, 4) == 0) {
    ...
    LOG_DEBUG("[CLIP-WS-SP-007] ServerProxy.cpp:parseMessage() -- received kMsgCClipboard from server");
    grabClipboard();
  }
    ...
    LOG_DEBUG("[CLIP-WS-SP-008] ServerProxy.cpp:parseMessage() -- received kMsgDClipboard from server");
    setClipboard();
  }
...
{
  LOG_DEBUG("[CLIP-WS-SP-001] ServerProxy.cpp:345 onGrabClipboard() -- sending kMsgCClipboard to server, id=%d", id);
...
{
  LOG_DEBUG("[CLIP-WS-SP-002] ServerProxy.cpp:352 onClipboardChanged() -- sending kMsgDClipboard to server, id=%d", id);
...
{
  LOG_DEBUG("[CLIP-WS-SP-004] ServerProxy.cpp:520 setClipboard() -- receiving clipboard from server");
  // parse
  ...
  if (r == TransferState::Started) {
    ...
    LOG_DEBUG("[CLIP-WS-SP-006] ServerProxy.cpp:setClipboard() -- forwarding to Client::setClipboard(), id=%d, size=%zu", id, dataCached.size());
