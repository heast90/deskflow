...
{
  LOG_DEBUG("[CLIP-CU-CL-001] Client.cpp:173 getClipboard() -- reading local clipboard, id=%d", id);
...
bool Client::leave()
{
  ...
  if (m_enableClipboard) {
    ...
    for (ClipboardID id = 0; id < kClipboardEnd; ++id) {
      LOG_DEBUG("[CLIP-CU-CL-007] Client.cpp:205 leave() -- sending clipboard %d to server, ownClipboard=%d, sentClipboard=%d", id, m_ownClipboard[id], m_sentClipboard[id]);
      sendClipboard(id);
    }
  }
...
{
  LOG_DEBUG("[CLIP-CU-CL-002] Client.cpp:213 setClipboard() -- writing to local clipboard, id=%d, ownClipboard=%d, sentClipboard=%d", id, m_ownClipboard[id], m_sentClipboard[id]);
...
{
  LOG_DEBUG("[CLIP-CU-CL-004] Client.cpp:220 grabClipboard() -- clipboard invalidated by server, id=%d, ownClipboard=%d, sentClipboard=%d", id, m_ownClipboard[id], m_sentClipboard[id]);
