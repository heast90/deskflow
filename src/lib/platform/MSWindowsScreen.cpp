...
{
  LOG_DEBUG("[CLIP-WS-MS-019] checkClipboards() -- polling ownership, m_ownClipboard=%d, isOwnedByDeskflow=%d", m_ownClipboard, MSWindowsClipboard::isOwnedByDeskflow());
...
void MSWindowsScreen::onClipboardChange()
{
  ...
  LOG_DEBUG("[CLIP-WS-MS-014] onClipboardChange() -- entry m_ownClipboard=%d, isOwnedByDeskflow=%d", m_ownClipboard, ownedByDeskflow);
  if (!ownedByDeskflow) {
    if (m_ownClipboard) {
      LOG_DEBUG("[CLIP-WS-MS-015] clipboard changed: lost ownership, marking m_ownClipboard=false");
      m_ownClipboard = false;
      sendClipboardEvent(EventTypes::ClipboardGrabbed, kClipboardClipboard);
      sendClipboardEvent(EventTypes::ClipboardGrabbed, kClipboardSelection);
    } else {
      LOG_DEBUG("[CLIP-WS-MS-018] external clipboard change detected, !m_ownClipboard, sending grab");
      sendClipboardEvent(EventTypes::ClipboardGrabbed, kClipboardClipboard);
      sendClipboardEvent(EventTypes::ClipboardGrabbed, kClipboardSelection);
    }
  } else if (!m_ownClipboard) {
    LOG_DEBUG("[CLIP-WS-MS-016] onClipboardChange() -- deskflow owns clipboard, isOwnedByDeskflow=%d, m_ownClipboard=%d, setting=true", ownedByDeskflow, m_ownClipboard);
    m_ownClipboard = true;
  } else {
    LOG_DEBUG("[CLIP-WS-MS-017] onClipboardChange() -- deskflow owns && m_ownClipboard already true, isOwnedByDeskflow=%d, m_ownClipboard=%d", ownedByDeskflow, m_ownClipboard);
  }
}
