...
PACKAGE_VERSION=$(LANG=C.UTF-8 xvfb-run deskflow --version | grep '^Deskflow' | cut -d' ' -f2 | sed -e 's/,//')
...
if [ "$PACKAGE_VERSION" = "${CHANGELOG_VERSION}" ]; then
    ...
    echo "deskflow: package version and changelog version must be matched"
